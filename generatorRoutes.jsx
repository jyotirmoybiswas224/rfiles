...
export const useSsrRequests = (userId, generatorId) => {
	...
	const cancelSubcontractorRequest = useCallback(
		async (ssrToCancel, cancelReason, terminationDate = null, terminationNote = "") => {
			...
			try {
				...
				const cancellationData = {
					...
					...(isTermination &&
						terminationDate && {
							...
							requester: `${transporterDoc.data().companyName} - ${transporterDoc.data().generalEmail}`,
				...
				showSuccessToastMessage(
					isTermination
						...
						: "Subcontractor request cancelled successfully"
				);
				return true;
			} catch (error) {
